      ...
      <div className="flex flex-wrap items-center justify-between gap-4 mb-5">
        <div>
          <h2 className="text-lg font-bold text-white flex items-center gap-2">
            ...
            Склад запчастин та матеріалів
          </h2>
          <p className="text-xs text-slate-400 mt-0.5">
            Контроль залишків деталей на складі та автоматичне списання
          </p>
        </div>
        ...
        <div className="relative w-full sm:w-64">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-1.5 bg-[#111827] border border-slate-800 rounded-lg text-xs text-white placeholder:text-slate-500 focus:outline-none focus:border-blue-500"
          />
        </div>
      </div>
          ...
          <table className="w-full text-left text-xs">
            <thead className="bg-slate-900/90 border-b border-slate-800 text-slate-400 font-semibold uppercase tracking-wider">
              <tr>
                ...
                <th className="py-3 px-4 text-right">Ціна закупівлі</th>
                <th className="py-3 px-4 text-right">Ціна продажу</th>
            ...
            <tbody className="divide-y divide-slate-800/60 text-slate-300">
              {loading ? (
                ...
                <tr>
                  <td colSpan={7} className="py-8 text-center text-slate-500">Запчастин не знайдено</td>
                </tr>
              ) : (
                filteredParts.map((part) => {
                  ...
                  return (
                    <tr key={part.id} className="hover:bg-slate-800/40 transition-colors">
                      <td className="py-3 px-4 font-mono font-bold text-blue-400">{part.sku}</td>
                      <td className="py-3 px-4 font-medium text-white">{part.name}</td>
                      <td className="py-3 px-4 text-slate-400">{part.category}</td>
                      <td className="py-3 px-4 text-center font-bold">
                        <span className={`px-2 py-0.5 rounded text-[11px] ${isLow ? 'bg-amber-500/15 text-amber-400 border border-amber-500/20' : 'bg-slate-800 text-slate-300'}`}>
                          {part.stockQuantity} шт.
                        </span>
                      </td>
                      <td className="py-3 px-4 text-right font-mono text-slate-400">{part.purchasePrice} ₴</td>
                      <td className="py-3 px-4 text-right font-mono font-bold text-white">{part.retailPrice} ₴</td>
